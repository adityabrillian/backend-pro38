...
// CEK RES STATUS 200 DI apiController landingPage
describe('API ENDPOINT TESTING', () => {
  it('GET Landing Page', (done) => {
    chai
      ...
      .end((err, res) => {
        expect(err).to.been.null; // Melihat ekspektasi error => jika error akan memperlihatkan errornya
        expect(res).to.have.status(200); // Melihat ekspektasi berhasil
        expect(res.body).to.be.an('object'); // Mau lihat yang bentuknya 'Object'
        ...
        expect(res.body.mostPicked).to.have.an('array'); // Melihat mostPicked bentuknya array dr mana???
        expect(res.body).to.have.property('categories');
        expect(res.body.categories).to.have.an('array');
        expect(res.body).to.have.property('testimonial');
        expect(res.body.testimonial).to.have.an('object');
        done(); // Tau selesainya kapan
      });
  });
  ...
  it('POST Booking Page', (done) => {
    ...
    const dataSample = {
      ...
      duration: 2,  // kenapa tidak bisa masuk dan undefined???
      bookingStartDate: '2-11-2020',
      bookingEndDate: '7-11-2020',
      firstName: 'Ikhwan',
      lastName: 'Irwan',
      email: 'ii@gmail.com',
      phoneNumber: '085732323124',
      accountHolder: 'irwan',
      bankFrom: 'BNI',
    }
    chai
      ...
      .end((err, res) => {
        ...
        console.log(res.body.booking.itemId.duration);
        done();
      });
  });
});
